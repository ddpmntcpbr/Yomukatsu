...
import {
  initializePostsListPaginationIndexAction,
  switchTabIndexAction,
} from "../postListPage/actions";
...
export const exchangeRegisteredAndReadingPost = (id) => {
  return async (dispatch) => {
    ...
    const apiUrl =
      ...
      "/registered/posts/exchange_registered_and_reading_post/" +
      String(id);
...
export const fetchPosts = () => {
  return async (dispatch) => {
    ...
    await axios
      .get(process.env.REACT_APP_API_V1_URL + "/reading/posts", {
        headers: {
          "access-token": localStorage.getItem("auth_token"),
          client: localStorage.getItem("client_id"),
          uid: localStorage.getItem("uid"),
    ...
    await axios
      .get(process.env.REACT_APP_API_V1_URL + "/registered/posts", {
        headers: {
          "access-token": localStorage.getItem("auth_token"),
          client: localStorage.getItem("client_id"),
          uid: localStorage.getItem("uid"),
    ...
    await axios
      .get(process.env.REACT_APP_API_V1_URL + "/completed/posts", {
        headers: {
          "access-token": localStorage.getItem("auth_token"),
          client: localStorage.getItem("client_id"),
          uid: localStorage.getItem("uid"),
...
export const saveReadingPost = (title, url, author, image, postItems) => {
  return async (dispatch) => {
    ...
    const apiUrl =
      process.env.REACT_APP_API_V1_URL +
      "/reading/change_status_from_reading_to_registered";
    ...
    await axios
      ...
      .then(() => {
        axios
          .post(process.env.REACT_APP_API_V1_URL + "/posts", data, {
            headers: {
              "access-token": localStorage.getItem("auth_token"),
              client: localStorage.getItem("client_id"),
              uid: localStorage.getItem("uid"),
...
export const updatePostItems = (id, initialPostItems, editedPostItems) => {
  return async (dispatch) => {
    ...
    const deletedPostItemsIds = _.difference(
      initialPostItemsIds,
      editedPostItemsIds
    );
