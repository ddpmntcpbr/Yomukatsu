...
const ResponsiveBottomNavigation = () => {
  ...
  const currentActiveBottomNavigationAction = (pathname) => {
    switch (true) {
      case /^(?=.*reading)(?=.*posts)/.test(pathname):
        return 0
      case /^(?=.*posts)(?=.*list)/.test(pathname):
        return 1
      case /^(?=.*registered)(?=.*posts)/.test(pathname):
        return 1
      case /^(?=.*completed)(?=.*posts)/.test(pathname):
        return 1
      case /edit/.test(pathname):
        return 2
      case /setting/.test(pathname):
        return 3
      default:
        return -1
    }
